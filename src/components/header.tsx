...
export default function Header() {
  ...
  const [isAuthOpen, setIsAuthOpen] = useState(false); // Новое состояние для модалки регистрации/входа
  const [authType, setAuthType] = useState<"login" | "register">("login"); // Какая вкладка открыта
  ...
  return (
    <header className="flex items-center justify-between px-6 py-3 bg-black shadow-md sticky top-0 z-50 border-b border-white/20 relative">
      ...
      <div className="hidden md:flex items-center space-x-4 mr-[150px]">
        <button
          className="px-5 py-2 text-white rounded-full cursor-pointer hover:bg-gray-500/30 transition"
          onClick={() => {
            setAuthType("login");
            setIsAuthOpen(true);
          }}
        ...
        <button
          className="px-5 py-3 bg-[#0077b6]/30 text-[#00b8ff]/90 font-normal rounded-[10px] cursor-pointer hover:bg-[#0077b6]/50 transition"
          onClick={() => {
      ...
      {isAuthOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/40 backdrop-blur-sm">
          <div className="relative bg-[#1a1a1a] rounded-2xl shadow-xl border border-white/10 p-6 w-[400px]">
            <button
              onClick={closeAuth}
              className="absolute top-3 right-3 text-gray-400 hover:text-white transition text-xl"
            >
            ...
            <form className="flex flex-col gap-4">
              {authType === "register" && (
                <input
                  ...
                  className="w-full bg-[#2a2a2a] border border-gray-700 text-white rounded-lg py-2 px-3 placeholder-gray-400 focus:outline-none focus:border-[#0077b6] transition"
                />
              )}
              <input
                type="email"
              ...
              <button
                type="submit"
                className="w-full py-3 bg-[#0077b6]/50 hover:bg-[#0077b6]/70 rounded-lg font-medium text-white transition"
              >
